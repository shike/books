...
本附录汇总第三卷 8 章提及的高频组织级场景,共 16 个提示词模板。每个模板对应一种成熟的组织动作,你可以按需复制到 WorkBuddy 最新版本(以官方为准,具体以 WorkBuddy 官方公告为准)的对话、自定义 Skill 或 自动化流程中直接运行。变量用尖括号 `<>` 包裹,跑前请把占位符替换为真实输入。"使用提示"标注了踩坑点和二次优化的方向,建议先跑一次基线再迭代。所有模板都遵循同一原则:角色 → 输入 → 流程 → 输出,这种四段式在第 2 章已被验证为最稳的写法。
...
输入:
对话记录文件:<file_path>
服务规范版本:<规范版本号,例如 WorkBuddy 最新版本(以官方为准)>
...
- **提示词**:
```text
你是事实核对助理。请将同一问题分别发送给至少 3 个不同模型(默认 其他主流闭源模型、主流闭源模型-Opus-4、Gemini-2.5-Pro),收集回答后输出对照表。
...
- **典型输出**:`{"model":"主流闭源模型-haiku-4","cost":"0.012","accuracy":0.92}`
...
- **使用频率**:每次数据出域前,或配置在 自动化流程中作为前置步骤。
- **所需 Skill**:`PIIScanner`。
- **提示词**:
```text
...
本附录共 16 个模板,覆盖第三卷第 3-8 章的核心场景,按"自定义 Skill / 多模型策略 / 企业系统对接 / ROI 论证 / 变革管理 / 数据安全 / 未来规划"七大类组织。模板不是终点,是起点——你跑过 2-3 次后,应把高频模板沉淀到自定义 Skill,让 WorkBuddy 替你记住四段式结构。所有占位符 `<>` 都按本卷统一约定:工作目录用相对路径、合规等级用 L1-L4、模型版本具体以你购买本书时 WorkBuddy 官方公告为准。第三卷正文若与本附录冲突,以最新版本 WorkBuddy 最新版本(以官方为准) 的实际行为为准;如需新增场景,可在配套 GitHub 仓库的 `templates/` 目录提交 PR。
